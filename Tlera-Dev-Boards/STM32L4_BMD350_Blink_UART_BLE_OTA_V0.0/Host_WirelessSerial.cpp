...
void WirelessSerial::evaluateCommand()
{
  switch(cmdWirelessSerial[CURRENTPORT])
  {
   ...
   case RECEIVE_OTA_DATA:                                                                                                 // CMD 13
     ...
     for(uint8_t i=0; i<OTA_DATA_BLOCK_SIZE; i++)
     {
...
{
  CURRENTPORT = STM32L4_BLE_UART;
  cmdWirelessSerial[STM32L4_BLE_UART] = 14;
  WirelessSerial::headSerialRequest(STM32L4_BLE_UART, 0, 1);
  WirelessSerial::serialize8(ack_nak_byte);
  WirelessSerial::tailSerialRequest(STM32L4_BLE_UART);
}
