...
void OTA::OTA_Update(uint32_t update_size, uint16_t update_blocks)
{
  ...
  } else
  {
    ...
    while(1)
    {
      ...
      {
        // It's been too long... Re-ACK the transaction
        WirelessSerial::OTA_Update_ACK_NAK(4);
        ACK_Start = millis();
        cycles++;
      }
 ...
 cycles = 0;
  for(uint16_t i=0; i<update_blocks; i++)
  {
    ...
    if(block == i)
    {
      ...
      STM32.flashProgram((imageStart + OTA_DATA_BLOCK_SIZE*i), OTA_bytes, OTA_DATA_BLOCK_SIZE);
  ...
  // You've written the full number of blocks, reset to activate the new sketch!
  delay(2000);
  STM32.reset();
}
