...
{
  CURRENTPORT = DRAGONFLY_BLE_UART;
  cmdBLEserial[DRAGONFLY_BLE_UART] = 12;                                                                                // Call CMD 12, "INITIATE_OTA_UPDATE" payload 6 bytes
  BLEserial::headSerialRequest(DRAGONFLY_BLE_UART, 0, 6);
  BLEserial::serialize32(OTA_Update_Size);
  BLEserial::serialize16(OTA_Update_Blocks);
  BLEserial::tailSerialRequest(DRAGONFLY_BLE_UART);
}
...
{
  CURRENTPORT = DRAGONFLY_BLE_UART;
  cmdBLEserial[DRAGONFLY_BLE_UART] = 13;                                                                                // Call CMD 13, "RECEIVE_OTA_DATA" payload OTA_DATA_BLOCK_SIZE+2 bytes
  BLEserial::headSerialRequest(DRAGONFLY_BLE_UART, 0, OTA_DATA_BLOCK_SIZE+2);
...
void BLEserial::SerialOpen(uint8_t port, uint32_t baud)
{
  ...
  switch (port)
  {
    case 0:
      ...
      break;
    case 1:
      Serial1.begin(baud, UartRxBuffer, 1024);
      break;
    case 2:
      Serial2.begin(baud, UartRxBuffer, 1024);
      break;
    case 3:
      Serial3.begin(baud, UartRxBuffer, 1024);
      break;
  }
}
