...
void setup()
{
  BLEserial::SerialOpen(DRAGONFLY_BLE_UART, 115200);                                                                // Initialize the BLE UART port
  delay(10000);
...
void flashFW()
{
  ...
  // Determine the file size and number of blocks
  ...
  // Wait for the remote IU board to respond; ACK/NAK byte set to 2 (Neither ACK nor NAK)
  BLEserial::serialCom();
  while(1)
  {
    if(ack_nak_byte == 4)
  ...
  {
    Serial.println("Writing OTA FW data to remote IU board...");
    for(uint16_t i=0; i<OTA_Update_Blocks; i++)
    {
      ...
      short_unsigned = i;
      OTA_bytes[0] = short_unsigned_byte[0]; OTA_bytes[1] = short_unsigned_byte[1];
